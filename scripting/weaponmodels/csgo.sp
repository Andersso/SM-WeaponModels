...
{
	if (g_ClientInfo[client].CustomWeapon)
	{
		int weaponIndex = g_ClientInfo[client].WeaponIndex;

		if (g_WeaponModelInfo[weaponIndex].BlockLAW)
		{
...
public void RefreshViewModel(any client)
{
	...
	if (g_ClientInfo[client].CustomWeapon)
	{
		...
		SetPlayerViewModel(client, 1, EntRefToEntIndex(g_ClientInfo[client].ViewModels[1]));
	}
}
